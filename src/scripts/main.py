...
clef_paths = {
    ...
    "bass": ["resources/template/clef/bass_1.jpg"],
}
...
accidental_paths = {
    ...
    "flat": ["resources/template/flat-line.png", "resources/template/flat-space.png"],
}
...
rest_paths = {
    ...
    "half": [
        "resources/template/rest/half_rest_1.jpg",
...
barline_paths = [
    "resources/template/barline/barline_1.jpg",
...
def get_ref_lengths(img):
    ...
    assert (
        line_spacing + line_width == width_spacing_sum
    ), "Estimated Line Thickness + Spacing doesn't correspond with Most Common Sum "
...
def find_staffline_rows(img, line_width, line_spacing):
    ...
    while current_row < iter_range:
        staff_lines = [
            row_black_pixel_histogram[j : j + line_width]
            for j in range(
                current_row,
        ...
        for line in staff_lines:
            ...
            staff_row_indices = [
                ...
                for j in range(
                    current_row,
                    current_row
                    + (num_stafflines - 1) * (line_width + line_spacing)
                    + 1,
                    line_width + line_spacing,
                )
            ]
    ...
    for i in range(len(all_staffline_vertical_indices)):
        ...
        for j in range(num_cols // 2):
            first_staff_rows_isolated = img[
                all_staffline_vertical_indices[i][0][
                    0
                ] : all_staffline_vertical_indices[i][4][line_width - 1],
                j,
            ]
...
if __name__ == "__main__":
    ...
    staffs = []
    half_dist_between_staffs = (
        all_staffline_vertical_indices[1][0][0]
        - all_staffline_vertical_indices[0][4][line_width - 1]
    ) // 2
    ...
    for i in range(len(all_staffline_vertical_indices)):
        ...
        staff_img = img[
            max(0, y - half_dist_between_staffs) : min(
            ...
            for primitive in staff_primitives:
                ...
                if type == "note" and note[0] in notes_to_modify:
                    new_note = (
                        ...
                        if accidental_type == "sharp"
                        else MIDI_to_pitch[pitch_to_MIDI[note] - 1]
                    )
